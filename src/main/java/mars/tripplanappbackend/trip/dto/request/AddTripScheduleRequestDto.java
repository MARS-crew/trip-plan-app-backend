...
/**
 * 일정 추가 화면에서 전달되는 요청 DTO입니다.
 * 컨트롤러에서는 경로 변수(tripId), 인증 사용자(usersId), 본문 입력값을 하나로 묶어
 * 서비스 계층으로 전달합니다.
 */
@Getter
@NoArgsConstructor(access = AccessLevel.PRIVATE)
@Schema(description = "일정 추가 요청 DTO")
public class AddTripScheduleRequestDto {
    ...
    /**
     * 컨트롤러 입력값을 서비스 계층용 DTO로 조합합니다.
     *
     * @param tripId 여행 PK
     * @param usersId 로그인 사용자 식별자
     * @param requestDto 컨트롤러에서 전달받은 요청 본문 DTO
     * @return 서비스 계층에서 사용하는 일정 추가 요청 DTO
     */
    public static AddTripScheduleRequestDto of(Long tripId, String usersId, AddTripScheduleRequestDto requestDto) {
        AddTripScheduleRequestDto serviceRequestDto = new AddTripScheduleRequestDto();
