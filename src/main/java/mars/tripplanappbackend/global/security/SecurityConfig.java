 ...
 * 외부 서비스(Swagger 등) 및 CORS 허용 정책을 관리합니다.
 */
@Configuration
@EnableWebSecurity
...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                // 세션을 사용하지 않는 REST API 환경이므로 CSRF 및 기본 폼 로그인 방식 비활성화
                ...
                // H2 콘솔 등 iframe을 사용하는 관리 도구와의 호환성을 위해 동일 도메인 내 프레임 허용
                .headers(headers -> headers.frameOptions(HeadersConfigurer.FrameOptionsConfig::sameOrigin))

                // 인증되지 않은 사용자가 보호된 리소스에 접근할 경우, 커스텀 응답 대신 표준 401 에러를 반환하도록 처리
    ...
    /**
     * cors 설정
     * * 프론트엔드 프레임워크와의 원활한 통신을 위해 모든 메서드와 헤더를 허용하며,
     * 클라이언트가 JWT 토큰을 인식할 수 있도록 'Authorization' 헤더를 외부로 노출함
     */
    @Bean
